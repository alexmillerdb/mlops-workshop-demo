...
def predict_batch(
    ...
    table = spark_session.table(input_table_name).select('ts', 'purchased', 'destination_id', 'user_id', 'user_latitude', 'user_longitude', 'booking_date')
    
    fe_client = FeatureEngineeringClient()

    prediction_df = fe_client.score_batch(
        model_uri=model_uri,
        df=table
    )
    output_df = (
