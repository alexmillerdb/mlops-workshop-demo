...
# MAGIC To run this demo, just select the cluster `dbdemos-feature-store-alex_miller` from the dropdown menu ([open cluster configuration](https://e2-demo-field-eng.cloud.databricks.com/#setting/clusters/0401-215330-kt38mhi6/configuration)). <br />
...
def create_online_table(table_name, pks, timeseries_key=None, sync="triggered"):
    w = WorkspaceClient()
...
create_online_table(f"{catalog}.{db}.user_features",                 ["user_id"], "ts", sync="continuous")
...
endpoint_name = "alexm_dbdemos_feature_store_endpoint_expert"
wc = WorkspaceClient()
